...
import { RoleUpdateInput } from "../api/role/RoleUpdateInput";

export const Role = (): React.ReactElement => {
  ...
  const [deleteEntity] = useMutation<TRole, AxiosError>(
    async (data) => {
  ...
  ] = useMutation<TRole, AxiosError, RoleUpdateInput>(async (data) => {
    const response = await api.patch(`${"/api/roles"}/${id}`, data);
    return response.data;
  });

  const handleSubmit = React.useCallback(
    (values: RoleUpdateInput) => {
      void update(values);
    },
    [update]
  );
